...
Any keywords mentioned in *italics* can be found (i.e. cmd+F) in the Max patch for convenience.
...
* *4synth/4synth2*
    ...
    * a simple crossfade between *4synth/4synth2* in order to produce sonic variations.
    ...
    * the *mono/stereo effect* experiments with the stereo depth. a value of 0 duplicates the L signal into both channels (i.e. L+L), while a value of 0 does the same with the R channel (R+R). A value of 50 restores the stereo depth field back to normal (L+R).
